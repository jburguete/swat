...
!!    Intrinsic: Sum, Sqrt, Max, Min, Abs
!!    SWAT: HQDAV
...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
   ...
   if (Sum(QMSI(1:nstep))<=0.01) then
      ...
      XL3 = ch_l(2,jrch) / 3.6
      XLT = XL3 / dthy
      XLS = 1000.* ch_l(2,jrch) * ch_s(2,jrch)
      ...
      do ii = L,NHY(inhyd)  !routing starts when inflow > 0.
         ...
         !INPUT:CBW,QI2,SSS,ZCH output: AI,ZI,CHW,FPW
   ...
   do ii = 1, nstep
      ...
      !INPUT:CBW,Q,SSS,ZCH output: AI,ZI,CHW,FPW
      call HQDAV(AI,CBW,QMS(ii),SSS,ZCH,ZI,CHW,FPW,jrch)
