...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
   ...
   if (hrupest(j) == 1) then
      do k = 1, npmx
         if (pst_lag(3,k,j) < 1.e-6) pst_lag(3,k,j) = 0.0
         !MFW, 3/3/12: Modified lagged pesticide to include decay in lag
         pst_lag(3,k,j) = (pst_lag(3,k,j) * decay_s(npno(k))) + lat_pst(k)
      end do
   end if
   ...
   if (hrupest(j) == 1) then
      do k = 1, npmx
         lat_pst(k) = pst_lag(3,k,j) * lat_ttime(j)
      end do
   end if
   ...
   if (hrupest(j) == 1) then
      do k = 1, npmx
         pst_lag(3,k,j) = pst_lag(3,k,j) - lat_pst(k)
      end do
   end if
